...
def get_news_assets(df):
    asset = df.assetCodes.str.extract('([A-Za-z]+\.(O|N|A|P|OB|OQ|UNKNOWN))', expand=True)
    asset.columns = ['assetCode', 'ext']
    df['assetCodesMap'] = asset['assetCode']
    del asset
    gc.collect()
    return df
news_train_df=get_news_assets(news_train_df)

def group_news(news_frame):
    news_frame['date'] = news_frame.time.dt.date  # Add date column
    
    aggregations = ['mean']
    gp = news_frame.groupby(['assetCodesMap', 'date']).agg(aggregations)
    gp.columns = pd.Index(["{}_{}".format(e[0], e[1]) for e in gp.columns.tolist()])
    gp.reset_index(inplace=True)
    # Set datatype to float32
    float_cols = {c: 'float32' for c in gp.columns if c not in ['assetCodesMap', 'date']}
    return gp.astype(float_cols)
news_train_df = group_news(news_train_df)
market_train_df = pd.merge(market_train_df, news_train_df, how='left', left_on=['date', 'assetCode'], 
                    right_on=['date', 'assetCodesMap'])
...
param_4 = {'max_depth': 10,  'random_state':1029, 'objective': 'binary:logistic', 'silent':True, 'eta':0.2, 'colsample_bylevel': 0.7113, \
'colsample_bytree': 0.9419, 'gamma': 1.7289, 'reg_alpha': 0.2135, 'reg_lambda': 0.2691, 'subsample': 0.8424}#0.75346



bst_3 = xgboost.train(param_4, dtrain, num_round, watchlist , feval=xgb_metric, verbose_eval=20, early_stopping_rounds=5,
                   maximize=True)
                   
'''                   
param_5 = {'max_depth': 10,  'random_state':1029, 'objective': 'binary:logistic', 'silent':True, 'eta':0.2,\
'colsample_bylevel': 0.3456903157357085, 'colsample_bytree': 0.5033010868836794, \
'gamma': 3.7838947522284725, 'reg_alpha': 0.7403966934819871, 'reg_lambda': 0.2992233528620154, 'subsample': 0.9290310077368328}


bst_4 = xgboost.train(param_5, dtrain, num_round, watchlist , feval=xgb_metric, verbose_eval=20, early_stopping_rounds=5,
                   maximize=True)
'''
train_probas = bst.predict(dtrain)
train_probas1 = bst_2.predict(dtrain)
train_probas2 = m.predict(dtrain)
train_probas3 = bst_3.predict(dtrain)
#train_probas4 = bst_4.predict(dtrain)

trainp = (train_probas + train_probas1 + train_probas2 + train_probas3) / 4

test_probas = bst.predict(dvalid)
test_probas1 = bst_2.predict(dvalid)
test_probas2 = m.predict(dvalid)
test_probas3 = bst_3.predict(dvalid)
#test_probas4 = bst_4.predict(dvalid)


testp = (test_probas + test_probas1 + test_probas2 + test_probas3) / 4
...
p_calibrated * 2 - 1
#.3809 -> 1.27373
def write_submission(model, model_2, model_3, model_4,env):#bst_4 is deleted
    ...
    for (market_obs_df, news_obs_df, predictions_template_df) in days:
        ...
        news_obs_df = get_news_assets(news_obs_df)
        news_obs_df = group_news(news_obs_df)
        market_obs_df = pd.merge(market_obs_process, news_obs_df, how='left', left_on=['date', 'assetCode'], 
                    right_on=['date', 'assetCodesMap'])
        
        market_obs_df = market_obs_df[market_obs_df['id']==day_id]
        ...
        preds4 = model_4.predict(test, ntree_limit=model_4.best_ntree_limit)
        #preds5 = model_5.predict(test, ntree_limit=model_5.best_ntree_limit) 
...
write_submission(bst, bst_2, m, bst_3,env)